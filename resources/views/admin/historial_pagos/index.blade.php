<x-layouts::app :title="'Historial de Pagos'">
    <div class="flex h-full w-full flex-1 flex-col gap-4 p-6">
        ...
        <div x-data="{ tab: 'lista' }">

            {{-- Tabs — solo mobile --}}
            <div class="flex md:hidden rounded-xl border border-zinc-200 dark:border-zinc-700 overflow-hidden mb-4">
                <button
                    type="button"
                    class="flex-1 py-2 text-sm font-medium transition"
                    x-on:click="tab = 'lista'"
                    x-bind:class="tab === 'lista' ? 'bg-white dark:bg-zinc-900 text-zinc-800 dark:text-white' : 'bg-zinc-50 dark:bg-zinc-800 text-zinc-400'"
                >
                    Listado
                </button>
                <button
                    type="button"
                    class="flex-1 py-2 text-sm font-medium transition"
                    x-on:click="tab = 'formulario'"
                    x-bind:class="tab === 'formulario' ? 'bg-white dark:bg-zinc-900 text-zinc-800 dark:text-white' : 'bg-zinc-50 dark:bg-zinc-800 text-zinc-400'"
                >
                    Nuevo pago
                </button>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                {{-- Columna izquierda: tabla / lista --}}
                <div class="lg:col-span-2 flex flex-col gap-3"
                    x-show="tab === 'lista'"
                    x-bind:class="{ 'hidden md:flex': tab !== 'lista', 'flex': tab === 'lista' }"
                >
                ...
                <div x-show="tab === 'formulario' || window.innerWidth >= 768"
                    x-bind:class="tab === 'formulario' ? 'block' : 'hidden md:block'">
